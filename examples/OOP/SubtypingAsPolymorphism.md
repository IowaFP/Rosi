...
We should be careful to avoid reading this result too broadly, as the actual theorem (Theorem 4.9 in the paper) is stated with respect to specific target calculi. As regards row polymorphism, it may be fair to interpret it as stating that we cannot give a type-only encoding of records with **only row polymorphism**, but even that should be understood with the caveat that this is limited to **row polymorphism as defined here**. This does not imply that presence polymorphism is necessary to give such a translation, since there may be some other language feature we could add to row polymorphism instead of presence polymorphism that would solve the problem. And depending what that feature is, it may be debatable whether what we have in that case is really **row polymorphism + X**, or just a different formulation of row polymorphism.
...
(I'll need to think about how Result 6.2 is possible notwithstanding Result 4.5)
...
* Examine Ningning Xie's paper on disjoint polymorphism and see how it relates to what we've explorede here.